...
int main() {
  ...
  h.onMessage([&map_waypoints_x, &map_waypoints_y, &map_waypoints_s,
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      auto jsn_serialed = hasData(data);

      if (jsn_serialed != "") {
        //auto jsn = json::parse(jsn_serialed);
        //json jsn = json::parse(jsn_serialed);
        json jsn = json::parse(jsn_serialed);

        string event = jsn[0].get<string>();
        ...
        if (event == "telemetry") {
          ...
          msgJson["next_y"] = road.next_y_vals;
