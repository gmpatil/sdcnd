...
void message_handler_twiddle(uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (s != "") {
      ...
      if (event == "telemetry") {
        ...
        if (steps < 1) {
          ...
          steer_value = pid.steering_angle;
...
// Manual tuning version.
void message_handler(uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
  ...
  if (length && length > 2 && data[0] == '4' && data[1] == '2') {
    ...
    if (s != "") {
      ...
      if (event == "telemetry") {
        ...
        if (steps % 10 == 0) {
          //std::cout << "Steps " << steps << std::endl;
          std::cout << "CTE: " << cte << " Speed:" << speed << " Steering angle:" << angle 
             << " Steering Value: " << steer_value << " throttle: " << throttle << std::endl;
        }
...
int main() {
  ...
  // Initialize the pid variable.
    ...
    // Manual tuning version
  // Initialize the pid variable.
    // successful round with < 15 speed
//    double kp = 35.0;
//    double kd = 450.0;
//    double ki = 0.06;
